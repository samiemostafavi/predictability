...
calc_and_plot_exact_agg(2, 64, c_mu, util, K, Llims, Lres, zlims, "#545454", "-", "s", 1, 1);
hold on;
calc_and_plot_approx_agg(2, 64, c_mu, util, K, Llims, Lres, zlims, "#174aa8", "--", "s", 1, 1);
hold on;

calc_and_plot_exact_agg(4, 64, c_mu, util, K, Llims, Lres, zlims, "#545454", "-", "o", 1, 1);
hold on;
calc_and_plot_approx_agg(4, 64, c_mu, util, K, Llims, Lres, zlims, "#174aa8", "--", "o", 1, 1);
hold on;

calc_and_plot_exact_agg(8, 64, c_mu, util, K, Llims, Lres, zlims, "#545454", "-", "x", 1, 1);
hold on;
calc_and_plot_approx_agg(8, 64, c_mu, util, K, Llims, Lres, zlims, "#174aa8", "--", "x", 1, 1);
hold on;
...
% export_fig geogeo1_evaluation3.eps -m10
% export_fig geogeo1_evaluation3.png -m10
% savefig( fig , 'geogeo1_evaluation3.fig' )
...
function result = calc_and_plot_exact(q, c_mu, factor, K, Llims, Lres, zlims, color, linestyle, marker, offset, hop)
    ...
    p = plot([Llims(1):Lres:Llims(2)],res_cf, '-o', 'DisplayName', sprintf('Exact $x=%d$, $K=%d$', q, K), 'LineWidth', 1.5,'MarkerIndices',offset:hop:length(res_cf));
    p.Color = color;
...
function result = calc_and_plot_approx(q, c_mu, factor, K, Llims, Lres, zlims, color, linestyle, marker, offset, hop)
    ...
    p = plot([Llims(1):Lres:Llims(2)],res_cf, '-o', 'DisplayName', sprintf('Approx. $x=%d$, $K=%d$', q, K), 'LineWidth', 1.5,'MarkerIndices',offset:hop:length(res_cf));
    p.Color = color;
    p.LineStyle = linestyle;
    p.Marker = marker;
    p.MarkerSize = 10;
...
function result = calc_and_plot_exact_agg(level, q, c_mu, factor, K, Llims, Lres, zlims, color, linestyle, marker, offset, hop)
    res_cf = [];
    mu = c_mu;
    lambda = mu*factor;
    rho = lambda/mu;
    for L=[Llims(1):Lres:Llims(2)]
        res_cf = [res_cf, mm1exact_agg(level, zlims, K, q, L, mu, lambda)];
    end
    p = plot([Llims(1):Lres:Llims(2)],res_cf, '-o', 'DisplayName', sprintf('Exact $a=%d$, $K/\\bar{K}=%d$', q/level, level), 'LineWidth', 1.5,'MarkerIndices',offset:hop:length(res_cf));
    p.Color = color;
    p.LineStyle = linestyle;
    p.Marker = marker;
    p.MarkerSize = 10;
...
function result = calc_and_plot_approx_agg(level, q, c_mu, factor, K, Llims, Lres, zlims, color, linestyle, marker, offset, hop)
    res_cf = [];
    mu = c_mu;
    lambda = mu*factor;
    rho = lambda/mu;
    for L=[Llims(1):Lres:Llims(2)]
        res_cf = [res_cf, approx_agg(level, q/level, L, mu, lambda)];
    end
    p = plot([Llims(1):Lres:Llims(2)],res_cf, '-o', 'DisplayName', sprintf('Approx. $a=%d$, $K/\\bar{K}=%d$', q/level, level), 'LineWidth', 1.5,'MarkerIndices',offset:hop:length(res_cf));
    p.Color = color;
    p.LineStyle = linestyle;
    p.Marker = marker;
    p.MarkerSize = 10;
...
function result = approx_agg(level, q, L, mu, lambda)
    ...
    newa = 1 - (beta^(level-1)/denominator)*lambda*(1-mu) - (1/denominator)*mu*(1-lambda);
    newb = sqrt(beta^(level-1))/denominator*b;
    newc = newb/(2*(newa+newb));
    result = (newbeta^((1-q)/2))*((1-sqrt(newbeta))^2)*((newa+newb)^L)/pi*theintegral(q, newbeta, L, newc);
end
